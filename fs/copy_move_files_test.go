...
func TestCopyFiles(t *testing.T) {
	...
	t.Run("copy directory recursively flattened", func(t *testing.T) {
		...
		err := CopyFiles([]string{srcDir}, destDir, CmRecursive, nil)
	...
	t.Run("copy directory non-recursively with structure preserved", func(t *testing.T) {
		...
		err := CopyFiles([]string{srcDir}, destDir, CmPreserveStructure, nil)
	...
	t.Run("copy directory recursively with structure preserved", func(t *testing.T) {
		...
		err := CopyFiles([]string{srcDir}, destDir, CmRecursive|CmPreserveStructure, nil)
	...
	t.Run("copy directory with extension filter flattened", func(t *testing.T) {
		...
		err := CopyFiles([]string{srcDir}, destDir, CmRecursive, []string{".txt"})
	...
	t.Run("copy directory with extension filter preserving structure", func(t *testing.T) {
		...
		err := CopyFiles([]string{srcDir}, destDir, CmRecursive|CmPreserveStructure, []string{".txt"})
	...
	t.Run("copy multiple directories with mixed flags", func(t *testing.T) {
		...
		err := CopyFiles([]string{srcDir1, srcDir2}, destDir, CmRecursive|CmPreserveStructure, nil)
	...
	t.Run("multiple extension filters", func(t *testing.T) {
		...
		err := CopyFiles([]string{srcDir}, destDir, CmRecursive, []string{".jpg", ".png"})
...
func TestMoveFiles(t *testing.T) {
	t.Run("move single file without extension filter", func(t *testing.T) {
		// Setup
		tempDir := t.TempDir()
		srcFile := filepath.Join(tempDir, "source.txt")
		destDir := filepath.Join(tempDir, "dest")
		...
		// Execute
		err := MoveFiles([]string{srcFile}, destDir, 0, nil)
	...
	t.Run("move single file with matching extension filter", func(t *testing.T) {
		tempDir := t.TempDir()
		srcFile := filepath.Join(tempDir, "source.txt")
		destDir := filepath.Join(tempDir, "dest")
		...
		err := MoveFiles([]string{srcFile}, destDir, 0, []string{".txt"})
	...
	t.Run("skip single file with non-matching extension filter", func(t *testing.T) {
		...
		err := MoveFiles([]string{srcFile}, destDir, 0, []string{".jpg"})
	...
	t.Run("move directory non-recursively flattened", func(t *testing.T) {
		tempDir := t.TempDir()
		srcDir := filepath.Join(tempDir, "src")
		destDir := filepath.Join(tempDir, "dest")
		...
		err := MoveFiles([]string{srcDir}, destDir, 0, nil)
		...
		assert.NoFileExists(t, filepath.Join(destDir, "file3.txt"))
		assert.NoDirExists(t, filepath.Join(destDir, "subdir"))
		assert.NoFileExists(t, filepath.Join(srcDir, "file1.txt"))
		assert.NoFileExists(t, filepath.Join(srcDir, "file2.txt"))
		assert.FileExists(t, filepath.Join(srcDir, "subdir", "file3.txt"))
	})

	t.Run("move directory recursively flattened", func(t *testing.T) {
		tempDir := t.TempDir()
		srcDir := filepath.Join(tempDir, "src")
		destDir := filepath.Join(tempDir, "dest")
		...
		err := MoveFiles([]string{srcDir}, destDir, CmRecursive, nil)
		...
		assert.NoDirExists(t, filepath.Join(destDir, "src"))
		assert.NoFileExists(t, filepath.Join(srcDir, "file1.txt"))
		assert.NoFileExists(t, filepath.Join(srcDir, "subdir", "file2.txt"))
	})

	t.Run("move directory non-recursively with structure preserved", func(t *testing.T) {
		tempDir := t.TempDir()
		srcDir := filepath.Join(tempDir, "src")
		destDir := filepath.Join(tempDir, "dest")
		...
		err := MoveFiles([]string{srcDir}, destDir, CmPreserveStructure, nil)

		require.NoError(t, err)
		assert.FileExists(t, filepath.Join(destDir, "src", "file1.txt"))
		assert.NoFileExists(t, filepath.Join(destDir, "src", "subdir", "file2.txt"))
		assert.NoFileExists(t, filepath.Join(srcDir, "file1.txt"))
		assert.FileExists(t, filepath.Join(srcDir, "subdir", "file2.txt"))
	})

	t.Run("move directory recursively with structure preserved", func(t *testing.T) {
		tempDir := t.TempDir()
		srcDir := filepath.Join(tempDir, "src")
		destDir := filepath.Join(tempDir, "dest")
		...
		err := MoveFiles([]string{srcDir}, destDir, CmRecursive|CmPreserveStructure, nil)
	...
	t.Run("move directory with extension filter flattened", func(t *testing.T) {
		tempDir := t.TempDir()
		srcDir := filepath.Join(tempDir, "src")
		destDir := filepath.Join(tempDir, "dest")
		...
		err := MoveFiles([]string{srcDir}, destDir, CmRecursive, []string{".txt"})

		require.NoError(t, err)
		assert.FileExists(t, filepath.Join(destDir, "file1.txt"))
		assert.NoFileExists(t, filepath.Join(destDir, "file2.jpg"))
		assert.NoFileExists(t, filepath.Join(srcDir, "file1.txt"))
		assert.FileExists(t, filepath.Join(srcDir, "file2.jpg"))
	})

	t.Run("move directory with extension filter preserving structure", func(t *testing.T) {
		tempDir := t.TempDir()
		srcDir := filepath.Join(tempDir, "src")
		destDir := filepath.Join(tempDir, "dest")
		...
		err := MoveFiles([]string{srcDir}, destDir, CmRecursive|CmPreserveStructure, []string{".txt"})
		...
		assert.NoFileExists(t, filepath.Join(srcDir, "file1.txt"))
		assert.FileExists(t, filepath.Join(srcDir, "file2.jpg"))
		assert.NoFileExists(t, filepath.Join(srcDir, "subdir", "file3.txt"))
	})

	t.Run("move multiple sources", func(t *testing.T) {
		tempDir := t.TempDir()
		srcFile1 := filepath.Join(tempDir, "file1.txt")
		srcFile2 := filepath.Join(tempDir, "file2.txt")
		destDir := filepath.Join(tempDir, "dest")
		...
		err := MoveFiles([]string{srcFile1, srcFile2}, destDir, 0, nil)
	...
	t.Run("move multiple directories with mixed flags", func(t *testing.T) {
		tempDir := t.TempDir()
		srcDir1 := filepath.Join(tempDir, "src1")
		srcDir2 := filepath.Join(tempDir, "src2")
		destDir := filepath.Join(tempDir, "dest")
		...
		err := MoveFiles([]string{srcDir1, srcDir2}, destDir, CmRecursive|CmPreserveStructure, nil)

		require.NoError(t, err)
		assert.FileExists(t, filepath.Join(destDir, "src1", "file1.txt"))
		assert.FileExists(t, filepath.Join(destDir, "src2", "file2.txt"))
		assert.NoFileExists(t, filepath.Join(srcDir1, "file1.txt"))
		assert.NoFileExists(t, filepath.Join(srcDir2, "file2.txt"))
	})
	...
	t.Run("multiple extension filters", func(t *testing.T) {
		...
		err := MoveFiles([]string{srcDir}, destDir, CmRecursive, []string{".jpg", ".png"})
		...
		assert.FileExists(t, filepath.Join(destDir, "file3.png"))
		assert.NoFileExists(t, filepath.Join(destDir, "file4.pdf"))
		assert.FileExists(t, filepath.Join(srcDir, "file1.txt"))
		assert.NoFileExists(t, filepath.Join(srcDir, "file2.jpg"))
		assert.NoFileExists(t, filepath.Join(srcDir, "file3.png"))
		assert.FileExists(t, filepath.Join(srcDir, "file4.pdf"))
	})
	...
	t.Run("deep nested directory structure", func(t *testing.T) {
		...
		require.NoError(t, err)
		assert.FileExists(t, filepath.Join(destDir, "src", "level1", "level2", "level3", "deep.txt"))
		assert.NoFileExists(t, filepath.Join(deepDir, "deep.txt"))
	})
	...
	t.Run("file name collision in flattened move", func(t *testing.T) {
		tempDir := t.TempDir()
		srcDir := filepath.Join(tempDir, "src")
		destDir := filepath.Join(tempDir, "dest")
		...
		require.NoError(t, err)
		assert.FileExists(t, filepath.Join(destDir, "file.txt"))
		assert.NoFileExists(t, filepath.Join(srcDir, "dir1", "file.txt"))
		assert.NoFileExists(t, filepath.Join(srcDir, "dir2", "file.txt"))
		// Note: One file will overwrite the other in flattened mode
	})
	...
	t.Run("empty sources slice", func(t *testing.T) {
		...
		err := MoveFiles([]string{}, destDir, 0, nil)
